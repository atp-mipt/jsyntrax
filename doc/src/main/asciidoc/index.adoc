...
If you use url maps, refer to <<Hyperlinked SVG>> section for the JSyntrax DSL which is a bit different from what was in original Syntrax (in short: you will have to replace `{}` with `[]` and add `jsyntrax` wrapper function).
...
JSyntrax diagrams are created using its own concise specification
language.
...
Being a syntax diagram generator, JSyntrax can describe its own
specification language. A specification is an optional `title`
followed by a single <<track,track>>, optionally wrapped into a
`jsyntrax()` call that attaches a <<Hyperlinked SVG,URL map>>.
In this and the following diagrams the box nodes are clickable and
lead to the definitions of the corresponding elements:

[syntrax,lang_specification,svg,opts=interactive]
....
jsyntrax(stack(
  line(opt('title', '(', "/'string'", ')')),
  line(choice('/track',
       line('jsyntrax', '(', '/track', opt(',', '/url-map'), ')')))
),
[
  'track': '.#track',
  'url-map': '.#urlmap'
...
A track is one of the diagram-building function calls. `loop()` and
`toploop()` take exactly two arguments, `indentstack()` takes an
indentation amount followed by the stacked lines, and the remaining
functions accept any number of arguments:

[[track]]
[syntrax,lang_track,svg,opts=interactive]
....
title('track')
jsyntrax(choice(
  line(choice('line', 'choice', 'opt', 'optx', 'stack', 'rightstack'),
       '(', opt(loop('/argument', ',')), ')'),
  line(choice('loop', 'toploop'), '(', '/argument', ',', '/argument', ')'),
  line('indentstack', '(', '/integer', ',', loop('/argument', ','), ')')
),
['argument': '.#argument'])
....

Each argument is a quoted string (single or double quotes), an
integer, the `None` literal denoting an empty node, or a nested track:

[[argument]]
[syntrax,lang_argument,svg,opts=interactive]
....
title('argument')
jsyntrax(choice("/'string'", '/integer', 'None', '/track'),
         ['track': '.#track'])
....

Finally, a URL map used in `jsyntrax()` maps node texts to hyperlink
targets:

[[urlmap]]
[syntrax,lang_urlmap,svg,opts=interactive]
....
title('url-map')
line('[', choice(':', loop(line("/'string'", ':', "/'string'"), ',')), ']')
....

Java-style `//` line comments and `/* */` block comments may be used
anywhere in a specification and are ignored.
...
====
Note that in JSyntrax, unlike Python, maps are enclosed in brackets `[]`, not braces `{}`.
====
